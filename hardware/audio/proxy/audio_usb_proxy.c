...
static const int pcm_format_preference_map[] = {PCM_FORMAT_S8, PCM_FORMAT_S16_LE, PCM_FORMAT_S24_LE,
                                                PCM_FORMAT_S24_3LE, PCM_FORMAT_S32_LE};
...
/**                                                                          **/
/******************************************************************************/
static int usb_extract_rates_from_ratestr(char* rates_str, struct usb_device_info* dev_config,
    ...
    if (strstr(rates_str, "continuous") != NULL) {
        ...
        for (i = 0; i < MAX_NUM_USB_SR; i++) {
            if (supported_usb_samplingrates[i] >= min_rate &&
                ...
                ALOGVV("%s: continuous sample rate supported_usb_samplingrates[%d] %d", __func__, i,
                       supported_usb_samplingrates[i]);
                *rates_mask |= (1 << i);
            }
        }
    } else {
        do {
            ...
            for (i = 0; i < MAX_NUM_USB_SR; i++) {
                if (supported_usb_samplingrates[i] == rate) {
                    ALOGVV("%s: sr %d, supported_usb_samplingrates[%d] %d", __func__, rate, i,
                           supported_usb_samplingrates[i]);
                    dev_config->rates[rate_size++] = supported_usb_samplingrates[i];
                    *rates_mask |= (1 << i);
                }
...
static int usb_get_profile_capability(void* proxy, int direction) {
    ...
    ALOGI("%s: for %s", __func__,
          (direction == USB_OUT) ? PLAYBACK_PROFILE_STR : CAPTURE_PROFILE_STR);

    /* generate device access path with card information */
    ret = snprintf(path, sizeof(path), "/proc/asound/card%u/stream0", card);
    if (ret < 0) {
    ...
    start_str =
            strstr(read_buf, ((direction == USB_OUT) ? PLAYBACK_PROFILE_STR : CAPTURE_PROFILE_STR));
    if (start_str == NULL) {
        ALOGE("%s: error %s section not found in usb config file", __func__,
              ((direction == USB_OUT) ? PLAYBACK_PROFILE_STR : CAPTURE_PROFILE_STR));
        ret = -EINVAL;
        goto done;
    }
    end_str =
            strstr(read_buf, ((direction == USB_OUT) ? CAPTURE_PROFILE_STR : PLAYBACK_PROFILE_STR));
    if (end_str > start_str) check = true;
    ...
    while (start_str != NULL) {
        ...
        if ((start_str == NULL) || (check && (start_str >= end_str))) {
            ...
            if (direction == USB_OUT) ALOGV("*USB_2 func_%s: done \n", __func__);
            break;
        }
        ...
        if ((format_str = (char*)malloc(size + 1)) == NULL) {
            ALOGE("%s: unable to allocate memory to hold bit width strings", __func__);
            ret = -EINVAL;
            free(usb_devconfig);
            break;
        }
        ...
        *channels_mask |= (1 << usb_devconfig->channels);
        ...
        if ((rate_str = (char*)malloc(size + 1)) == NULL) {
            ALOGE("%s: unable to allocate memory to hold sample rate strings", __func__);
            ret = -EINVAL;
            free(usb_devconfig);
            break;
        }
        ...
        if (ret < 0) {
            ALOGE("%s: error unable to get sample rate values", __func__);
            free(usb_devconfig);
            continue;
        }
...
static void usb_remove_device_info(void* proxy, int direction) {
    ...
    list_for_each_safe(node, auxi, usb_devlist) {
        ...
        if (devinfo_node) {
            ALOGI("%s: USB_Device[%s] %d: Info", __func__,
                  ((direction == USB_OUT) ? "PLAYBACK" : "CAPTURE"), ++count);
            list_remove(node);
            free(devinfo_node);
        }
    }
...
static void usb_print_device_info(void* proxy, int direction) {
    ...
    list_for_each(node, usb_devlist) {
        devinfo_node = node_to_item(node, struct usb_device_info, node);
        if (devinfo_node) {
            ALOGI("USB_Device[%s] %d: Info", ((direction == USB_OUT) ? "PLAYBACK" : "CAPTURE"),
                  ++count);
...
static int usb_get_best_matching_format(struct listnode* usb_devlist,
    ...
    list_for_each(node, usb_devlist) {
        ...
        if (devinfo_node) {
            ...
            ALOGVV("%s: USB fmt(%d) stream fmt(%d) selected fmt(%d)", __func__, cur_format,
                   stream_format, selected_format);
            if ((cur_format == stream_format) ||
                (cur_format == PCM_FORMAT_S24_3LE && stream_format == PCM_FORMAT_S24_LE)) {
                selected_format = cur_format;
                ALOGI("%s: found matching fmt(%d) stream fmt(%d)", __func__, selected_format,
                      stream_format);
                goto exit;
                ...
                selected_format = cur_format;
                ALOGI("%s: found better matching fmt(%d) stream fmt(%d)", __func__, selected_format,
                      stream_format);
            }
        }
    }
...
static unsigned int usb_get_best_matching_channels(struct listnode* usb_devlist, int format,
    ...
    list_for_each(node, usb_devlist) {
        ...
        if (devinfo_node) {
            ...
            ALOGVV("%s: USB fmt(%d)ch(%d) stream fmt(%d)ch(%d) selected ch(%d)", __func__,
                   cur_format, cur_channels, format, stream_channels, selected_channels);
            if (cur_format != format) continue;
            if (cur_channels == stream_channels) {
                selected_channels = cur_channels;
                ALOGI("%s: found matching ch(%d) stream ch(%d)", __func__, selected_channels,
                      stream_channels);
                goto exit;
                ...
                selected_channels = cur_channels;
                       ...
                       ((cur_channels > selected_channels) && (stream_channels > cur_channels))) {
                selected_channels = cur_channels;
                ALOGI("%s: found better matching ch(%d) stream ch(%d)", __func__, selected_channels,
                      stream_channels);
            }
        }
    }
...
static unsigned int usb_get_best_matching_samplerate(struct listnode* usb_devlist, int format,
    ...
    list_for_each(node, usb_devlist) {
        ...
        if (devinfo_node) {
            cur_format = devinfo_node->format;
            cur_channels = devinfo_node->channels;
            ALOGVV("%s: USB fmt(%d)ch(%d) stream fmt(%d)ch(%d)rate(%d) selected rate(%d)", __func__,
                   cur_format, cur_channels, format, channels, stream_rate, selected_rate);
            if ((cur_format != format) || (cur_channels != channels)) continue;
            for (i = 0; i < devinfo_node->rate_size; i++) {
                ALOGVV("%s: usb next rate(%d) selected rate(%d)", __func__, devinfo_node->rates[i],
                       selected_rate);
                if (devinfo_node->rates[i] == stream_rate) {
                    selected_rate = devinfo_node->rates[i];
                    ALOGI("%s: found matching rate(%d) stream rate(%d)", __func__, selected_rate,
                          stream_rate);
                    goto exit;
                } else if (selected_rate == 0) {
                    selected_rate = devinfo_node->rates[i];
                    ALOGI("%s: initial updated rate(%d) stream rate(%d)", __func__, selected_rate,
                          stream_rate);
                    ...
                    selected_rate = devinfo_node->rates[i];
                    ALOGI("%s: found better matching rate(%d) stream rate(%d)", __func__,
                          selected_rate, stream_rate);
                }
            }
        }
    }
...
static bool usb_get_best_matching_config(struct listnode* usb_devlist,
                                         enum pcm_format stream_format,
                                         unsigned int stream_channels, unsigned int stream_rate,
     ...
     */
    ALOGI("proxy-%s: stream config SR(%d) CH(%d) FMT(%d)", __func__, stream_rate, stream_channels,
          stream_format);

    best_pcmconfig->format = usb_get_best_matching_format(usb_devlist, stream_format);
    best_pcmconfig->channels =
            usb_get_best_matching_channels(usb_devlist, best_pcmconfig->format, stream_channels);
    best_pcmconfig->rate = usb_get_best_matching_samplerate(usb_devlist, best_pcmconfig->format,
                                                            best_pcmconfig->channels, stream_rate);

    ALOGI("proxy-%s: USB best matching config SR(%d) CH(%d) FMT(%d)", __func__,
          best_pcmconfig->rate, best_pcmconfig->channels, best_pcmconfig->format);
...
static unsigned int usb_get_max_channel(struct listnode* usb_devlist) {
    ...
    list_for_each(node, usb_devlist) {
        ...
        if (devinfo_node) {
            cur_channels = devinfo_node->channels;
            if (cur_channels > selected_channels) selected_channels = cur_channels;
        }
    }
...
char* usb_get_format_strs(const unsigned int formats_mask) {
    ...
    while (count--) {
        ...
        if (buffSize - curStrLen <
            strlen(supported_usb_format_strs[idx]) + (numEntries != 0 ? 2 : 1)) {
...
char* usb_get_channel_count_strs(void* proxy_usb, const unsigned int channels_mask, int direction) {
    ...
    char* const* const chans_strs = ((direction == USB_OUT) ? supported_usb_out_channel_strs
                                                            : supported_usb_in_channel_strs);
    unsigned int idx =
            ((direction == USB_OUT) ? FCC_2 : 1);  // OUT start from Stereo, IN start from Mono
    ...
    for (; idx <= MAX_NUM_USB_CHANNELS; idx++) {
        if (idx >= min && idx <= max) {
            ...
            // account for both the null, and potentially the bar.
            if (buffSize - curStrLen <
                strlen(supported_usb_channel_strs[idx]) + (numEntries != 0 ? 2 : 1)) {
                /* we don't have room for another, so bail at this point rather than
                 * return a malformed rate string
                 */
                break;
...
char* usb_get_sample_rate_strs(const unsigned int rates_mask) {
    ...
    while (count--) {
        ...
        // account for both the null, and potentially the bar.
        if (buffSize - curStrLen <
            strlen(supported_usb_samplingrate_strs[idx]) + (numEntries != 0 ? 2 : 1)) {
            /* we don't have room for another, so bail at this point rather than
             * return a malformed rate string
             */
            break;
...
static int usb_audio_gain_load_xml(void* proxy, int usb_card) {
    ...
    if (aproxy_usb->usb_pid == USB_BUNDLE_WHITE_PID) {
        ...
        ALOGI("proxy-%s: USB White Bundle GainControl XML [%s] loading", __func__, gain_mixer_path);
    } else if (aproxy_usb->usb_pid == USB_BUNDLE_GRAY_HEADSET_PID ||
               aproxy_usb->usb_pid == USB_BUNDLE_GRAY_HEADPHONE_PID) {
        strcpy(gain_mixer_path, USB_BUNDLE_GRAY_GAIN_XML_MIXER_PATH);
        ALOGI("proxy-%s: USB Gray Bundle GainControl XML [%s] loading", __func__, gain_mixer_path);
    } else {
        strcpy(gain_mixer_path, USB_BUNDLE_DEFAULT_GAIN_XML_MIXER_PATH);
        ALOGI("proxy-%s: USB Default Bundle GainControl XML [%s] loading", __func__,
              gain_mixer_path);
    }
    ...
    if (!aproxy_usb->usb_ar) {
        ALOGE("proxy-%s: failed to init audio route for USB Gain usb_card: %d", __func__, usb_card);
        ret = -EINVAL;
    }
...
static void usb_audio_gain_control_enable(void* proxy) {
    ...
    if (!aproxy_usb->usb_gaincontrol_needed &&
        ...
        if (aproxy_usb->usb_out_pcm_card != -1 || aproxy_usb->usb_in_pcm_card != -1) {
            usb_card = ((aproxy_usb->usb_out_pcm_card != -1) ? aproxy_usb->usb_out_pcm_card
        ...
        memset(path, 0, sizeof(path));
        ret = snprintf(path, sizeof(path), "/proc/asound/card%u/usbid", usb_card);
        if (ret < 0) {
        ...
        fd = open(path, O_RDONLY);
        if (fd < 0) {
            ALOGE("%s: failed to open usbid file %s error: %d", __func__, path, errno);
            goto err;
        }
        ...
        ALOGI("proxy-%s: USB Device VID: 0x%x PID: 0x%x", __func__, aproxy_usb->usb_vid,
              aproxy_usb->usb_pid);
    ...
    if (aproxy_usb && aproxy_usb->usb_out_connected) {
        if (aproxy_usb->usb_out_status == false) {
            ...
            aproxy_usb->usb_out_pcm = pcm_open(aproxy_usb->usb_out_pcm_card,
                                               aproxy_usb->usb_out_pcm_device, flags, ppcmconfig);
            if (aproxy_usb->usb_out_pcm && !pcm_is_ready(aproxy_usb->usb_out_pcm)) {
                /* pcm_open does always return pcm structure, not NULL */
                ALOGE("%s-%s: PCM Device is not ready with Sampling_Rate(%u) error(%s)!", "usb_out",
                      __func__, ppcmconfig->rate, pcm_get_error(aproxy_usb->usb_out_pcm));
                goto err_open;
            }
            ...
            if (dummy && aproxy_usb->usb_out_pcm &&
                ...
                ALOGI("%s-%s: The opened USB Out PCM Device is %s with SR(%d), CC(%d), Format(%d)",
                      "usb-out", __func__, pcm_path, ppcmconfig->rate, ppcmconfig->channels,
                      ppcmconfig->format);

                aproxy_usb->usb_out_status = true;
            } else {
                ALOGE("%s-%s: USB Out PCM Device write failed %s", "usb-out", __func__,
                      ((aproxy_usb->usb_out_pcm) ? pcm_get_error(aproxy_usb->usb_out_pcm)
                                                 : "Error"));
                goto err_open;
            }
...
static void usb_open_in_proxy(struct audio_proxy_usb* aproxy_usb) {
    ...
    if (aproxy_usb && aproxy_usb->usb_in_connected) {
        ...
        aproxy_usb->usb_in_pcm = pcm_open(aproxy_usb->usb_in_pcm_card,
                                          aproxy_usb->usb_in_pcm_device, flags, ppcmconfig);
        if (aproxy_usb->usb_in_pcm && !pcm_is_ready(aproxy_usb->usb_in_pcm)) {
            /* pcm_open does always return pcm structure, not NULL */
            ALOGE("%s-%s: PCM Device is not ready with Sampling_Rate(%u) error(%s)!", "usb_in",
                  __func__, ppcmconfig->rate, pcm_get_error(aproxy_usb->usb_in_pcm));
            goto err_open;
        }

        snprintf(pcm_path, sizeof(pcm_path), "/dev/snd/pcmC%uD%u%c", aproxy_usb->usb_in_pcm_card,
                 aproxy_usb->usb_in_pcm_device, 'c');

        ALOGVV("%s-%s: USB In PCM Device opened %s with SR(%d), CC(%d), Format(%d)", "usb-in",
               __func__, pcm_path, ppcmconfig->rate, ppcmconfig->channels, ppcmconfig->format);
        ...
        if (aproxy_usb->usb_in_pcm && pcm_start(aproxy_usb->usb_in_pcm) == 0) {
            ALOGI("%s-%s: USB In PCM Device opened/started %s with SR(%d), CC(%d), Format(%d)",
                  "usb-in", __func__, pcm_path, ppcmconfig->rate, ppcmconfig->channels,
                  ppcmconfig->format);
        } else {
            ALOGE("%s-%s: PCM Device(%s) with SR(%u) CC(%d) Format(%d) cannot be started as "
                  "error(%s)",
                  "usb-in", __func__, pcm_path, ppcmconfig->rate, ppcmconfig->channels,
                  ppcmconfig->format,
...
void proxy_usb_playback_prepare(void* proxy_usb, bool set_default) {
    ...
    if (set_default) {
        ...
        usb_get_best_matching_config(
                &aproxy_usb->usbplayback_devlist, aproxy_usb->active_playback_picked_format,
                aproxy_usb->active_playback_picked_channels,
                aproxy_usb->active_playback_picked_rate, &aproxy_usb->usb_out_active_pcmconfig);
        aproxy_usb->usb_out_cpcall_prepared = false;
    } else {  // CPCall fixed or supported configuration
        usb_get_best_matching_config(&aproxy_usb->usbplayback_devlist, DEFAULT_USB_MEDIA_FORMAT,
                                     DEFAULT_USB_MEDIA_CHANNELS, DEFAULT_USB_MEDIA_SAMPLING_RATE,
    ...
    ALOGI("proxy-%s: configured USB Out Proxy SR(%d) CH(%d) FMT(%d)", __func__,
          aproxy_usb->usb_out_active_pcmconfig.rate, aproxy_usb->usb_out_active_pcmconfig.channels,
...
int proxy_usb_getparam_playback_stream(void* proxy_usb, void* query_params, void* reply_params) {
    ...
    if (aproxy_usb->usb_out_connected) {
        ...
        if (str_parms_has_key(query, AUDIO_PARAMETER_STREAM_SUP_CHANNELS)) {
            char* channels_list = usb_get_channel_count_strs(
                    aproxy_usb, aproxy_usb->usb_out_channels_mask, USB_OUT);
            if (channels_list) {
...
void proxy_usb_capture_prepare(void* proxy_usb, bool set_default) {
    ...
    if (set_default) {
        ...
        usb_get_best_matching_config(&aproxy_usb->usbcapture_devlist, DEFAULT_USB_MEDIA_FORMAT,
    ...
    ALOGI("proxy-%s: configured USB InProxy SR(%d) CH(%d) FMT(%d)", __func__,
          aproxy_usb->usb_in_active_pcmconfig.rate, aproxy_usb->usb_in_active_pcmconfig.channels,
...
int proxy_usb_getparam_capture_stream(void* proxy_usb, void* query_params, void* reply_params) {
    ...
    if (aproxy_usb->usb_in_connected) {
        ...
        if (str_parms_has_key(query, AUDIO_PARAMETER_STREAM_SUP_CHANNELS)) {
            char* channels_list = usb_get_channel_count_strs(
                    aproxy_usb, aproxy_usb->usb_in_channels_mask, USB_IN);
            if (channels_list) {
                str_parms_add_str(reply, AUDIO_PARAMETER_STREAM_SUP_CHANNELS, channels_list);
                free(channels_list);
            }
...
bool proxy_usb_out_pick_best_pcmconfig(void* proxy_usb, struct pcm_config cur_pcmconfig) {
    ...
    ALOGI("%s: current config rate[%d] format[%d] channels[%d]", __func__, cur_pcmconfig.rate,
          cur_pcmconfig.format, cur_pcmconfig.channels);
    ...
    ALOGI_IF(is_updated, "%s: Selected config rate[%d] format[%d] channels[%d]", __func__,
             aproxy_usb->active_playback_picked_rate, aproxy_usb->active_playback_picked_format,
             aproxy_usb->active_playback_picked_channels);
...
bool proxy_usb_out_reconfig_needed(void* proxy_usb) {
    ...
    usb_get_best_matching_config(&aproxy_usb->usbplayback_devlist,
                                 ...
                                 aproxy_usb->active_playback_picked_channels,
                                 aproxy_usb->active_playback_picked_rate, &sup_pcmconfig);
    ...
    ALOGI_IF(reconfig_needed, "%s: need reconfig rate[%d] format[%d] channels[%d]", __func__,
             aproxy_usb->active_playback_picked_rate, aproxy_usb->active_playback_picked_format,
             aproxy_usb->active_playback_picked_channels);
...
void proxy_usb_set_gain(void* proxy_usb, char* path_name) {
    ...
    if (!aproxy_usb->usb_gaincontrol_needed) return;
...
int proxy_usb_set_parameters(void* proxy_usb, void* parameters) {
    ...
    if (ret >= 0) {
        if ((audio_devices_t)val == AUDIO_DEVICE_OUT_USB_DEVICE ||
            ...
            ALOGI("proxy-%s: connected USB Out Device with card %d / device %d", __func__, card,
                  device);
            ...
            ALOGI("proxy-%s: connected USB In Device with card %d / device %d", __func__, card,
                  device);
            ...
            if (!aproxy_usb->usb_in_connected && (card != -1 && device != -1)) {
                ...
                usb_get_best_matching_config(&aproxy_usb->usbcapture_devlist,
                                             DEFAULT_USB_MEDIA_FORMAT, DEFAULT_USB_MEDIA_CHANNELS,
                                             DEFAULT_USB_MEDIA_SAMPLING_RATE,
                                             &aproxy_usb->usb_in_active_pcmconfig);
                ...
                if (aproxy_usb->usb_in_active_pcmconfig.rate == DEFAULT_USB_MEDIA_SAMPLING_RATE &&
                    ...
                    ALOGI("proxy-%s: USB-In direct path supported CH[%d]", __func__,
        ...
        if (aproxy_usb->usb_out_connected || aproxy_usb->usb_in_connected) {
