...
int proxy_a2dp_start(void) {
    ...
    if (aproxy_a2dp) {
        ...
        if (aproxy_a2dp->cur_status == AUDIO_A2DP_STATUS_STANDBY) {
...
int proxy_a2dp_stop(void) {
    ...
    if (aproxy_a2dp) {
        ...
        if (aproxy_a2dp->cur_status == AUDIO_A2DP_STATUS_STARTED) {
            ret = a2dpHal->a2dp_suspend_stream();  // suspend stream during connecting bt-a2dp
            if (ret == 0) {
                aproxy_a2dp->prev_status = aproxy_a2dp->cur_status;
...
int proxy_a2dp_suspend(bool flag) {
    ...
    if (aproxy_a2dp) {
        ...
        if (flag == true) {
            ...
            if (ret == 0 && aproxy_a2dp->cur_status == AUDIO_A2DP_STATUS_SUSPENDED) {
                aproxy_a2dp->prev_status = aproxy_a2dp->cur_status;
                aproxy_a2dp->cur_status = AUDIO_A2DP_STATUS_STANDBY;
                ALOGI("proxy-%s: Transit to %s from %s", __func__,
                      a2dpstatus_table[aproxy_a2dp->cur_status],
...
int proxy_a2dp_open(void) {
    ...
    if (aproxy_a2dp) {
        ...
        if (aproxy_a2dp->cur_status == AUDIO_A2DP_STATUS_INIT) {
...
int proxy_a2dp_deinit(void) {
    ...
    if (aproxy_a2dp) {
        ...
        ALOGI("proxy-%s: Transit to %s", __func__, a2dpstatus_table[aproxy_a2dp->cur_status]);
